...
plotHeatmap <- function(geneset,
  ...
  if(cluster_rows == FALSE){
    idx <- match(geneset,input$SYMBOL)
    input <- input[idx,]
  }
  if(rownames=="SYMBOL"){
...
plotVenn <- function(comparisons,
  ...
  vennDiagram(venn,counts.col = "black",circle.col = c("#007090","#ff7a69"),cex=c(0.7,0.5,0.2))
}
...
timecourse <- function(timepoints){
  results <- as.data.frame(matrix(0, ncol = 0, nrow = nrow(norm_anno))) 
  for(i in 1:length(timepoints)){
    tmp <- DEresults[grep(paste("_", timepoints[i], "$", sep=""), names(DEresults))][[1]]@results
    
    results[,i] <- ifelse(tmp$padj > 0.05 & tmp$log2FoldChange > log(2,2), "n.s.(up)",
                          ifelse(tmp$padj > 0.05 & tmp$log2FoldChange < -log(2,2), "n.s.(down)",
                                 ifelse(tmp$padj > 0.05 &  tmp$log2FoldChange < log(2,2) & tmp$log2FoldChange > -log(2,2), "n.s.(noChange)",
                                        ifelse(tmp$padj < 0.05 & tmp$log2FoldChange>log(2,2),"up",
                                               ifelse(tmp$padj < 0.05 & tmp$log2FoldChange< (-log(2,2)),"down",
                                                      ifelse(tmp$padj < 0.05 &  abs(tmp$log2FoldChange) < log(2,2),"sig.(noChange)",
                                                             "undetermined"))))))
  }
  colnames(results) <- timepoints
  rownames(results) <- rownames(norm_anno)
  results$profile <- apply(results[ ,c(1:ncol(results))] , 1 , paste , collapse = "_" )
  return(results)
}

# Function for enrichment of activation signatures from Xue et al.
enrichActivation <- function(comparison,
                             pCorrection = "bonferroni", # choose the p-value adjustment method
                             pvalueCutoff = 0.05, # set the unadj. or adj. p-value cutoff (depending on correction method)
                             qvalueCutoff = 0.05, # set the q-value cutoff (FDR corrected)
                             showMax = 20,
  ...
  results$Activationup <- as.data.frame(enricher(DE_up,
                                                 TERM2GENE=ActivationSignatures,
                                                 universe = norm_anno$SYMBOL,  
                                                 pAdjustMethod = pCorrection,
                                                 pvalueCutoff  = pvalueCutoff,
                                                 qvalueCutoff = qvalueCutoff))
  if(nrow(results$Activationup)>0){results$Activationup$Enrichment <- paste("Macrophage Activation enrichment for genes upregulated in ",comparison,sep="")}
  
  results$Activationdown <- as.data.frame(enricher(DE_down,
                                                   TERM2GENE=ActivationSignatures,
                                                   universe = norm_anno$SYMBOL,  
                                                   pAdjustMethod = pCorrection,
                                                   pvalueCutoff  = pvalueCutoff,
                                                   qvalueCutoff = qvalueCutoff))
  if(nrow(results$Activationdown)>0){results$Activationdown$Enrichment <- paste("Macrophage Activation enrichment for genes downregulated in ",comparison,sep="")}
